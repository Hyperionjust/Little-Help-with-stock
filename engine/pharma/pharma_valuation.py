...
def estimate_peak_sales(asset, ta="other"):
    ...
    if patients is not None and price is not None:
        # 自下而上测算：患者数×渗透率×定价×疗程——非默认
        return patients * pen * price * course, pen, comp, False
    if asset.get("peak_sales") is not None:
        return asset.get("peak_sales"), pen, comp, False
    # 兜底默认：给可编辑的 illustrative 非零峰值销售
    # 用了 TA 分层默认常数——这是数量级示意，不是估值。返回 is_default=True 供上层警告。
    return DEFAULT_PEAK_SALES_BY_TA.get(ta, DEFAULT_PEAK_SALES_BY_TA["other"]), pen, comp, True
...
def compute_asset_rnpv(asset, clinical_rate, base_year=2026, margin=0.35):
    ...
    peak, pen, comp, peak_is_default = estimate_peak_sales(asset, ta)
    ...
    if asset.get("cashflows"):
        ...
        return {
            ...
            "peak_is_default": peak_is_default,
...
def sensitivity(asset, clinical_rate, base_rnpv, base_year=2026):
    ...
    peak, _, _, _ = estimate_peak_sales(asset, _ta)
    if peak:
        out["peak_sales"] = {
...
def build_catalysts(raw, as_of, horizon_months=24):
    """未来 horizon_months 内的催化剂。

    修复记录（方案第七部分医药项）：原版把 ClinicalTrials.gov 返回的**全部**
    primary_completion_date 都塞进催化剂——恒瑞实测吐出 300 条横跨 2012-2030，
    其中大量是已完成的历史试验。催化剂的定义是"未来会发生、能推动价值重估的事件"，
    过期的不是催化剂。此处按 as_of 过滤到 [今天, 今天+24个月] 窗口。
    """
    ...
    try:
        ...
        base = _dt.date.today()
    hi = base + _dt.timedelta(days=int(horizon_months * 30.44))
    ...
    return {"window": f"{base.isoformat()} ~ {hi.isoformat()}", "items": cats,
...
def compute_pharma(raw, wacc_general=0.09):
    ...
    # 峰值销售诚实性：统计有多少资产的峰值销售落在默认常数上
    n_default = sum(1 for a in pipeline_assets
                    if compute_asset_rnpv(a, clinical_rate).get("peak_is_default"))
    n_assets = len(pipeline_assets)
    ...
    pharma = {
        ...
        "illustrative_warning": ({
            "check": "peak_sales_all_default",
            "n_default": n_default, "n_assets": n_assets,
            "message": (f"{n_default}/{n_assets} 个管线资产的峰值销售取自 TA 分层默认常数"
                        f"（3000/2000/1500 百万），非自下而上的患者数×定价测算。"
                        f"公司 rNPV 因此是**数量级示意，绝非目标价**——免费源无商业化"
                        f"预测数据，精确峰值销售需接入 Cortellis/Evaluate 或人工录入")
        } if n_default >= max(1, n_assets * 0.5) else None),
    }
    return pharma
