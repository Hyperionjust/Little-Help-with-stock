...
# 修复（方案第七部分）：迈瑞 300760 是医疗**器械**制造商，无药物管线，
# 不应激活 rNPV。器械/医疗服务与创新药的估值范式完全不同。
PHARMA_SYMBOLS = {"600276", "603259", "BGNE", "688180", "01801", "02359", "06160"}
# "医疗"/"health" 过泛（覆盖器械、服务、连锁医院），移出；保留明确指向药物的词
PHARMA_NAME_KW = ["医药", "制药", "疫苗", "药业", "pharma", "therapeutics",
                  "biotech", "biopharm"]
# 器械/服务排除词：命中则**不**判为 pharma，即使其它关键词也命中
NON_PHARMA_KW = ["器械", "设备", "诊断", "医疗服务", "医院", "体外诊断", "device", "diagnostics"]
...
def resolve(raw_input, industry_hint=None, name_hint=None):
    ...
    # 器械/服务排除：命中排除词则否决（迈瑞类误判的兜底）
    if is_pharma and any(k in hay for k in NON_PHARMA_KW):
        is_pharma = False
